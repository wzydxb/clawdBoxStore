...
# 推 config.yaml，但保留盒子自己的 api_key（每台盒子 token 不同，不能覆盖）
EXISTING_KEY=$(_ssh "grep 'api_key:' $H/config.yaml 2>/dev/null | head -1 | awk '{print \$2}'" 2>/dev/null || true)
_scp "$L/config/hermes.yaml" "$TARGET:$H/config.yaml"
if [ -n "$EXISTING_KEY" ] && [ "$EXISTING_KEY" != '""' ] && [ "$EXISTING_KEY" != "null" ]; then
  _ssh "python3 -c \"
import re
c = open('$H/config.yaml').read()
c = re.sub(r'api_key:.*', 'api_key: $EXISTING_KEY', c)
open('$H/config.yaml','w').write(c)
\""
  ok "AGENTS.md / config.yaml（api_key 已保留盒子原有 token）"
else
...
_ssh "
  SVC=\$HOME/.config/systemd/user/chromium-vnc.service
  CHROME_CMD='/usr/bin/chromium --remote-debugging-port=9222 --start-fullscreen --no-first-run --disable-metrics --disable-metrics-reporting --metrics-recording-only --disable-field-trial-config --no-pings --load-extension=/root/opencli-extension'
  if [ -f \"\$SVC\" ]; then
    sed -i \"s|^ExecStart=.*|ExecStart=\$CHROME_CMD|\" \"\$SVC\"
    systemctl --user daemon-reload 2>/dev/null
    systemctl --user enable chromium-vnc.service 2>/dev/null
    systemctl --user restart chromium-vnc.service 2>/dev/null
    echo 'chromium-vnc patched'
  else
    mkdir -p \$HOME/.config/systemd/user
    cat > \"\$SVC\" << 'SVCEOF'
[Unit]
Description=Chromium Browser in VNC
...
ExecStart=/usr/bin/chromium --remote-debugging-port=9222 --start-fullscreen --no-first-run --disable-metrics --disable-metrics-reporting --metrics-recording-only --disable-field-trial-config --no-pings --load-extension=/root/opencli-extension
Restart=always
RestartSec=10
...
SVCEOF
    systemctl --user daemon-reload 2>/dev/null
    systemctl --user enable --now chromium-vnc.service 2>/dev/null
    echo 'chromium-vnc created'
  fi
