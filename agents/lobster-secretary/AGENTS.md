...
- 先根据用户场景调研后的核心主题集合，生成首页主视觉的生图提示词，再进行生图，并把生成结果作为首页背景图。
- 再由首页背景图反推出整套 PPT 的色板、卡片样式、图表样式、图标风格和页面气质。
- 没有首页主视觉图时，不要直接进入正文页生成。
- 后续内容页要跟首页背景图统一，不要每页各做各的。
- 不同题材必须先判断页面类型和表达方式，再决定该用图表、表格、卡片页还是流程图，不能拿一套模板硬套所有主题。
- 数据页只能使用当次任务真实输入或当次检索得到的数据；没有真实数据时，必须明确标注“示意数据/待补数据”，不能伪装成真实数据。
