#!/usr/bin/env python3
# open-link.py — Clawbox Open Link（获取页面详细内容）
...
APP_ID = os.getenv("CLAWBOX_AUTOGLM_APP_ID", "100003")
APP_KEY = os.getenv("CLAWBOX_AUTOGLM_APP_KEY", "")
URL = "https://autoglm-api.zhipuai.cn/agentdr/v1/assistant/skills/open-link"
TOKEN_URL = os.getenv("CLAWBOX_AUTOGLM_TOKEN_URL", "")


def require_runtime_config() -> None:
    if not APP_KEY:
        print("ERROR: 缺少环境变量 CLAWBOX_AUTOGLM_APP_KEY。")
        sys.exit(1)
    if not TOKEN_URL:
        print("ERROR: 缺少环境变量 CLAWBOX_AUTOGLM_TOKEN_URL。")
        sys.exit(1)
