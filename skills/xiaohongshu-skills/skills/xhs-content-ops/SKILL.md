...
本技能使用桌面截图技能（`/root/.openclaw/workspace/skills/desktop-screenshot.md`）在关键操作节点截取浏览器画面发送给用户。复合流程中截图时机遵循各子技能的规则：

- **搜索结果返回后**：截图搜索页面（同 xhs-explore）。
- **笔记详情加载后**：截图笔记页面，辅助分析（同 xhs-explore）。
- **发布预览 / 发布完成后**：截图发布页面（同 xhs-publish）。
- **评论发送成功后**：截图确认评论已出现（同 xhs-interact）。
