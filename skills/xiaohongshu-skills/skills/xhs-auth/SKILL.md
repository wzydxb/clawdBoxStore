...
**第二步** — 调用桌面截图技能（`/root/.openclaw/workspace/skills/desktop-screenshot.md`）只截取浏览器窗口（不要截整个桌面）发送给用户。这一步在展示二维码链接后立即执行，不可跳过，不可与其他步骤合并。
...
**第四步** — 登录成功后（`{"logged_in": true}`），调用桌面截图技能只截取浏览器窗口（不要截整个桌面）发送给用户。截图完成后再向用户汇报登录结果，不可先汇报再截图。
