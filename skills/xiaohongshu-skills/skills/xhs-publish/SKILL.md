...
1. **填写表单后** — `fill-publish` / `fill-publish-video` 完成后，必须截图让用户预览排版效果再决定是否发布。
2. **发布成功后** — `publish` / `click-publish` 成功后，必须截图让用户确认内容已发布。
3. **长文排版后** — `long-article` 一键排版完成后，必须截图展示排版效果。
4. **选择模板后** — `select-template` 应用模板后，必须截图展示模板效果。
...
# 步骤 1.5（必须）: 调用桌面截图技能截取浏览器预览画面发送给用户，不可跳过

# 步骤 2: 通过 AskUserQuestion 让用户确认浏览器中的预览

# 步骤 3a: 用户确认发布
python scripts/cli.py click-publish
# 发布成功后（必须）: 调用桌面截图技能截取浏览器页面发送给用户确认已发布
...
**截图（必须）**：排版完成后，必须调用桌面截图技能截取浏览器页面展示排版效果，不可跳过。
...
**截图（必须）**：模板应用后，必须调用桌面截图技能截取浏览器页面展示模板效果，不可跳过。
