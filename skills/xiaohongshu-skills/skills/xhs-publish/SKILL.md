...
# 步骤 2: fill-publish 返回的 JSON 包含 browser_screenshot，用 Read 工具读取截图展示给用户

# 步骤 3: 通过 AskUserQuestion 让用户确认浏览器中的预览

# 步骤 4a: 用户确认发布
python scripts/cli.py click-publish
# click-publish 返回的 JSON 包含 browser_screenshot，用 Read 工具读取截图展示给用户
...
# 步骤 2: fill-publish-video 返回的 JSON 包含 browser_screenshot，用 Read 工具读取截图展示给用户
...
long-article 返回的 JSON 包含 `browser_screenshot`，**必须用 message 工具的 media 参数将截图发送给用户**。
...
select-template 返回的 JSON 包含 `browser_screenshot`，**必须用 message 工具的 media 参数将截图发送给用户**。
...
click-publish 返回的 JSON 包含 `browser_screenshot`，**必须用 message 工具的 media 参数将截图发送给用户**。
