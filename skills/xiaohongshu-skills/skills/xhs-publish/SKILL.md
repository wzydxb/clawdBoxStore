...
long-article 返回的 JSON 包含 `browser_screenshot`，**必须用 Read 工具读取截图展示给用户**。
...
select-template 返回的 JSON 包含 `browser_screenshot`，**必须用 Read 工具读取截图展示给用户**。
...
click-publish 返回的 JSON 包含 `browser_screenshot`，**必须用 Read 工具读取截图展示给用户**。
