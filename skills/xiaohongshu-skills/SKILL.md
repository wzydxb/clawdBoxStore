...
**必须调用**桌面截图技能（`/root/.openclaw/workspace/skills/desktop-screenshot.md`）截取浏览器画面发送给用户。这是强制要求，不可省略。

**以下节点必须执行截图，各子技能在对应环节不得跳过：**

| 环节 | 必须截图的时机 |
|------|---------------|
| 登录 | 二维码展示后（必须）、登录成功后（必须） |
| 发布预览 | `fill-publish` / `fill-publish-video` 填写完成后（必须） |
| 发布完成 | `publish` / `click-publish` 成功后（必须） |
| 长文排版 | `long-article` 排版后（必须）、`select-template` 选择模板后（必须） |
| 搜索结果 | `search-feeds` 返回结果后（必须） |
| 笔记详情 | `get-feed-detail` 加载后（必须） |
| 互动确认 | 评论发送成功后（必须） |
