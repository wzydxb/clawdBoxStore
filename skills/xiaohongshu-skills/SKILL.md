...
CLI 命令成功执行后会**自动截取浏览器可视区域**，截图路径通过 JSON 输出的 `browser_screenshot` 字段返回。

**当 JSON 输出包含 `browser_screenshot` 字段时，必须使用 message 工具的 media 参数将该截图文件发送给用户。** 这是让用户看到浏览器实际画面的唯一方式，不可省略。将截图和结果文字合并在同一条 message 中发送，不要分成多条消息。
