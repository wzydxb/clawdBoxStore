...
5. **复合运营**（"竞品分析 / 账号诊断 / 首页分析 / 选题策划 / 热点追踪 / 爆款拆解 / 批量互动 / 一键创作"）→ 执行 `xhs-content-ops` 技能。
...
- 操作频率不宜过高，保持合理间隔。
- **浏览器截图是每个 CLI 命令执行后的必要步骤**，与"发布前必须用户确认"同等重要，详见下方规则。

## 🔴 浏览器截图规则（与技能边界同级，违反视为流程错误）

每次执行以下 CLI 命令并成功返回后，**下一个动作必须是调用桌面截图技能**（`/root/.openclaw/workspace/skills/desktop-screenshot.md`）**只截取浏览器窗口**发送给用户。不要截取整个桌面，只需要浏览器窗口的内容。截图完成前，禁止执行任何后续 CLI 命令或向用户汇报结果。

**触发截图的 CLI 命令清单：**

| CLI 命令 | 截图目的 |
|----------|----------|
| `check-login`（未登录，返回二维码时） | 让用户看到浏览器中的二维码 |
| `wait-login`（登录成功时） | 让用户确认登录成功 |
| `get-qrcode` | 让用户看到浏览器中的二维码 |
| `fill-publish` | 让用户预览排版效果，**截图后才能询问用户是否发布** |
| `fill-publish-video` | 让用户预览排版效果，**截图后才能询问用户是否发布** |
| `publish` | 让用户确认已成功发布 |
| `click-publish` | 让用户确认已成功发布 |
| `long-article` | 让用户看到排版效果 |
| `select-template` | 让用户看到模板效果 |
| `search-feeds` | 让用户看到搜索结果页面 |
| `get-feed-detail` | 让用户看到笔记的实际图片和排版 |
| `post-comment` | 让用户确认评论已出现在页面上 |
| `reply-comment` | 让用户确认回复已出现在页面上 |

**执行模式（固定顺序，不可调换）：**

```
步骤 1: 执行 CLI 命令
步骤 2: 调用桌面截图技能，只截取浏览器窗口发送给用户  ← 不可跳过
步骤 3: 向用户汇报结果 / 继续下一步操作
```

如果跳过步骤 2 直接到步骤 3，视为流程错误。
...
组合多步骤完成运营工作流：首页推荐流分析、账号五维诊断、选题策划、爆款拆解与复刻、竞品分析、热点追踪、内容创作、互动管理。分析结论沉淀到 `knowledge-base/` 知识库。
