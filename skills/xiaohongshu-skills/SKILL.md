...
每次执行以下 CLI 命令并成功返回后，**下一个动作必须是调用桌面截图技能**（`/root/.openclaw/workspace/skills/desktop-screenshot.md`）截取当前浏览器画面发送给用户。截图完成前，禁止执行任何后续 CLI 命令或向用户汇报结果。
...
```
步骤 1: 执行 CLI 命令
步骤 2: 调用桌面截图技能，截取浏览器画面发送给用户  ← 不可跳过
