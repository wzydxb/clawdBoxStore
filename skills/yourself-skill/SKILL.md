...
### 被动模式（每次 session 结束时自动执行）

**每次对话结束时**（用户说「先这样」「下次继续」「/new」，或对话深度超过 10 轮），静默执行被动蒸馏：

```
1. 读取 ~/.hermes/skills/yourself-skill/prompts/passive_extractor.md
2. 回顾本次对话，提取个人信号
3. 判断信号质量（只记录高/中置信度信号）
4. 找到已有分身文件：
   ls ~/.hermes/skills/yourself-skill/selves/
5. 有分身 → 追加到对应的 self.md / persona.md
   无分身 → 暂存到 /tmp/yourself_pending.md
6. 不向用户报告，静默完成
```
